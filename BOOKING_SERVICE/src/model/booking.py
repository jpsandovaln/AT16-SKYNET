...
class Booking:
    def booking_create(self, str_json: any) -> str:
        inserted_data: any = booking.insert_one(str_json)
    ...
    def booking_read_all(self) -> list:
        # Dictionary that save all results
        all_results: list = []

        filter: dict = {"delete": 0}
        result: any = booking.find(filter)
    ...
    def booking_read_specific_name(self, name: str) -> str:
        filter: dict = {"delete": 0}
        result: any = booking.find_one(name, filter)
    ...
    def booking_read_specific_id(self, id_booking: any) -> str:
        ...
        if result:
            # Convert ObjectId to string
            result['_id']: list = str(result['_id'])
        else:
            raise BookingException("Invalid specific ID", "603", "AT16-ERROR-106", "booking_read_specific_id")
        return result

    def booking_update(self, id_booking: any, str_json: any) -> str:
        id_object: any = ObjectId(id_booking)
        result: any = booking.find_one_and_update({'_id': id_object}, {'$set': str_json},
    ...
    def booking_delete(self, id_booking: any) -> str:
        id_object: any = ObjectId(id_booking)
        result: any = booking.find_one_and_update({'_id': id_object}, {'$set': {'delete': 1}},
                                             return_document=ReturnDocument.AFTER)
